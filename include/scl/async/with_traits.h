...
namespace scl {
    namespace async {
        ...
        struct with_traits {
             ...
             */
            template <class F>
            auto operator()(SCL_MAYBE_UNUSED_ATTR Resource& resource, F&& delegate) -> SCL_RETURN(
        ...
        struct with_traits<std::mutex> {
            template <class F>
            auto operator()(std::mutex& mutex, F&& delegate) -> decltype(std::forward<F>(delegate)()) {
                SCL_MAYBE_UNUSED_ATTR std::lock_guard<decltype(mutex)> g{mutex};
