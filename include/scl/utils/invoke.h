...
namespace scl {
    namespace utils {
        namespace details {
            ...
            struct invoker<Callable, scl::meta::enable_if_t<scl::meta::exists<decltype(&Callable::operator())>()>> {
                Callable callable;

                template <class... Args>
                inline constexpr auto operator()(Args&&... args) -> SCL_RETURN(
                    callable(std::forward<Args>(args)...)
                )
            };
        }

        template <class F, class... Args, class = scl::meta::enable_if_t<
            scl::meta::exists<details::invoker<F>>()
        >>
        inline constexpr auto invoke(F&& fn, Args&&... args) -> SCL_RETURN(
            details::invoker<F>{std::forward<F>(fn)}(std::forward<Args>(args)...)
        )
